...
def build_atlas_samples(vertices, faces, active_face_indices, texture_size):
    ...
    # Chunk temporary triangle/sample tensors to cap peak memory.
    ...
    print(
        f"[atlas] {atlas_size}x{atlas_size}, {len(points):,} samples", flush=True)
    return (atlas_size, tile_size, triangle_uvs, points,
            barycentrics, face_ids, pixels, masks)
...
def main():
    ...
    parser.add_argument("--input", required=True,
                        help="Input mesh path. e.g) data/mesh.obj")
    parser.add_argument(
        "--output", help="Output mesh path. e.g) results/mesh_simplified.obj")
